...
Nella nostra \emph{design library} ci siamo concentrati sul progetto di NeoVim per poter individuare sia degli esempi di buona che di cattiva progettazione da cui prendere spunto.\\
...
Questa situazione crea molta confusione nel momento in cui lo sviluppatore vuole dare il proprio contributo e, controllando cosa ci sia da fare e/o correggere, si trova davanti ad una miriade di labels con i colori pi\`u disparati e visualizzate in modo disorganizzato e poco coerente.
...
\item la scelta di una struttura \textbf{gerarchica} \`e nata dal fatto che chiunque abbia usato un gestore di file su un computer ha chiara l'idea di cartella, tramite la quale \`e possibile organizzare i propri dati in modo coerente. Questo ci permette di utilizzare un concetto non nuovo in un contesto diverso dal quale l'utente si aspetta di trovarlo, rendendo l'utilizzo della nostra funzionalit\`a ragionevolmente immediato.
\end{itemize}
...
I paragrafi seguenti si prefiggono l'obiettivo di trarre delle valide ed oggettive conclusioni.
...
Dall'analisi delle risposte fornite dagli intervistati e dalla nostra esperienza possiamo affermare che la principale difficolt\`a nel contribuire a progetti open source sia causata dagli strumenti per la gestione dei pregetti a disposizione dei team di sviluppo. Ipotesi confermata dal nostro studio, infatti non abbiamo trovato significativi impedimenti nella struttura del progetto di NeoVim in s\`e e per s\`e.\\
